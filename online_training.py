...
parser.add_argument("--window_steps", type=int, default=0,
                    help="Sliding window size (number of recent online steps to keep). 0=disabled (original behavior)")
parser.add_argument("--confidence_threshold", type=float, default=0.0,
                    help="Confidence gating threshold for pseudo-labels. 0=disabled (original behavior)")
...
####################### Stage 2: Online Training #######################
    ...
    # --- Sliding window setup (Optimization 5) ---
    # pseudo_history stores (x_data, y_labels) for each recent online step
    if window_steps > 0:
        pseudo_history_x = deque(maxlen=window_steps)
        pseudo_history_y = deque(maxlen=window_steps)
        # Also maintain a parallel deque for y_train_detection (unflipped labels for evaluate)
        pseudo_history_y_det = deque(maxlen=window_steps)

    # Track confidence gating stats
    total_accepted = 0
    total_pseudo = 0
    ...
    while len(x_test_left_epoch) > 0:
        ...
        # --- Confidence gating (Optimization 1): get predictions + confidence ---
        if confidence_threshold > 0:
            predict_label, confidence_scores = evaluate(
                normal_temp, normal_recon_temp, x_train_this_epoch, y_train_detection,
                x_test_this_epoch, 0, model, get_confidence=True)
        else:
        ...
        # --- Confidence gating: filter low-confidence pseudo-labels ---
        y_test_pred_this_epoch = predict_label
        if confidence_threshold > 0:
            confident_mask = confidence_scores >= confidence_threshold
            confident_mask_t = torch.from_numpy(confident_mask).to(x_test_this_epoch.device)
            num_confident = int(confident_mask.sum())
            total_accepted += num_confident
            total_pseudo += len(confident_mask)
            accept_rate = total_accepted / total_pseudo
            ...
            print(f'[Stage2] seed={seed+i}, step={count}/{total_online_steps} '
                  ...
                  f'Accept={num_confident}/{len(confident_mask)} ({accept_rate:.1%})')

            # Filter: only keep confident samples
            x_confident = x_test_this_epoch[confident_mask_t]
            y_confident_det = y_test_pred_this_epoch[confident_mask]  # unflipped, for detection (cpu tensor)
            y_confident = y_confident_det.clone() if isinstance(y_confident_det, torch.Tensor) else torch.tensor(y_confident_det.copy())

            # Apply label flip on confident samples only
            if flip_percent > 0 and len(y_confident) > 0:
                num_flip = int(flip_percent * len(y_confident))
                if num_flip > 0:
                    flip_idx = np.random.choice(len(y_confident), num_flip, replace=False)
                    y_confident_np = y_confident.cpu().numpy() if isinstance(y_confident, torch.Tensor) else np.array(y_confident)
                    y_confident_np[flip_idx] = 1 - y_confident_np[flip_idx]
                    y_confident = torch.tensor(y_confident_np)
        else:
            print(f'[Stage2] seed={seed+i}, step={count}/{total_online_steps} '
                  f'({100*processed/total_online_samples:.1f}%) | '
                  f'Acc={batch_acc:.4f}  Prec={batch_prec:.4f}  '
            ...
            x_confident = x_test_this_epoch
            y_confident_det = y_test_pred_this_epoch  # unflipped
            y_confident = y_confident_det.clone() if isinstance(y_confident_det, torch.Tensor) else torch.tensor(y_confident_det.copy())

            # Apply label flip (original behavior)
            if flip_percent > 0 and len(y_confident) > 0:
                num_flip = int(flip_percent * len(y_confident))
                if num_flip > 0:
                    flip_idx = np.random.choice(len(y_confident), num_flip, replace=False)
        ...
        # --- Sliding window (Optimization 5) vs original accumulation ---
        if window_steps > 0 and len(x_confident) > 0:
            # Add current step's data to window
            pseudo_history_x.append(x_confident.to(device))
            pseudo_history_y.append(y_confident.to(device))
            y_det_tensor = y_confident_det if isinstance(y_confident_det, torch.Tensor) else torch.tensor(y_confident_det)
            pseudo_history_y_det.append(y_det_tensor.to(device))

            # Construct training set: original labeled data + windowed pseudo-labels
            x_train_this_epoch = torch.cat([online_x_train] + list(pseudo_history_x))
            y_train_this_epoch = torch.cat([online_y_train] + list(pseudo_history_y))
            y_train_detection = torch.cat([online_y_train.clone()] + list(pseudo_history_y_det))
        elif len(x_confident) > 0:
            # Original accumulation behavior (no window)
            y_det_tensor = y_confident_det if isinstance(y_confident_det, torch.Tensor) else torch.tensor(y_confident_det)
            y_train_detection = torch.cat((y_train_detection.to(device), y_det_tensor.to(device)))
            x_train_this_epoch = torch.cat((x_train_this_epoch.to(device), x_confident.to(device)))
            y_train_this_epoch_temp = y_train_this_epoch.clone()
            y_train_this_epoch = torch.cat((y_train_this_epoch_temp.to(device), y_confident.to(device)))

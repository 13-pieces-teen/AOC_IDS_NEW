...
def evaluate(normal_temp, normal_recon_temp, x_train, y_train, x_test, y_test, model, get_confidence=False, en_or_de=False, return_predictions=False):
    ...
    if mu1_fit > mu2_fit:
        gaussian1 = dist.Normal(mu1_fit, abs(sigma1_fit))
        gaussian2 = dist.Normal(mu2_fit, abs(sigma2_fit))
    else:
        gaussian2 = dist.Normal(mu1_fit, abs(sigma1_fit))
        gaussian1 = dist.Normal(mu2_fit, abs(sigma2_fit))
    ...
    if mu3_fit > mu4_fit:
        gaussian3 = dist.Normal(mu3_fit, abs(sigma3_fit))
        gaussian4 = dist.Normal(mu4_fit, abs(sigma4_fit))
    else:
        gaussian4 = dist.Normal(mu3_fit, abs(sigma3_fit))
        gaussian3 = dist.Normal(mu4_fit, abs(sigma4_fit))
    ...
    # Combined confidence: take the max confidence from encoder/decoder (whichever was chosen)
    combined_confidence = np.maximum(y_test_pro_en, y_test_pro_de)
    ...
    if not isinstance(y_test, int):
        ...
        if get_confidence:
            return result_encoder, result_decoder, result_final, combined_confidence
        return result_encoder, result_decoder, result_final
    else:
        if get_confidence:
            return y_test_pred_no_vote, combined_confidence
